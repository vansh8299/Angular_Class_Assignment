...
export class TaskComponent implements OnInit {
  ...
  @ViewChild('pickerStartDate') pickerStartDate!: MatDatepicker<any>;
  ...
  saveEdit(task: Task, field: string, value: any) {
    ...
    if (index > -1) {
      if (field === 'name' && !value.trim()) {
        this.showSnackbar('Task name cannot be empty.');
        return;
      }

      if (field === 'startDate' && !value) {
        this.showSnackbar('Start date cannot be empty.');
        return;
      }

      if (field === 'startTime' && !value.trim()) {
        this.showSnackbar('Start time cannot be empty.');
        return;
      }

      if (field === 'endDate' && !value) {
        this.showSnackbar('End date cannot be empty.');
        return;
      }

      if (field === 'endTime' && !value.trim()) {
        this.showSnackbar('End time cannot be empty.');
        return;
      }

      if (field === 'startDate' && task.endDate && value > task.endDate) {
        this.showSnackbar('Start date cannot be after end date.');
        return;
      }

      if (field === 'endDate' && task.startDate && value < task.startDate) {
        this.showSnackbar('End date cannot be before start date.');
        return;
      }

      if (field === 'startTime' && task.endTime && value > task.endTime) {
        this.showSnackbar('Start time cannot be after end time.');
        return;
      }

      if (field === 'endTime' && task.startTime && value < task.startTime) {
        this.showSnackbar('End time cannot be before start time.');
        return;
      }

      // Temporarily set the new value for calculation
      ...
      if (
        newDuration === '0 hours 0 minutes' ||
        moment.duration(newDuration).asHours() > 24
      ) {
        this.showSnackbar(
          'Invalid duration. Duration cannot be zero or exceed 24 hours.'
        );
        (this.list[index] as any)[field] = originalValue;
        return;
      }

      // Apply the new value and duration
  ...
  calculateDurationForTask(task: Task): string {
    ...
    if (hours < 0 || minutes < 0 || duration.asHours() > 24) {
      this.showSnackbar(
        'Invalid duration. Duration cannot be negative or exceed 24 hours.'
      );
  ...
  calculateDuration(): string {
    ...
    if (startDate && endDate) {
      ...
      if (hours < 0 || minutes < 0 || duration.asHours() > 24) {
        ...
        this.form.get('duration')?.setValue('0 hours 0 minutes');
        return '0 hours 0 minutes';
      }
